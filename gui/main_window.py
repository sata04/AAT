...
class MainWindow(QMainWindow):
    ...
    def update_dataset_selector(self):
        ...
        if self.is_comparing:
            ...
            self.dataset_selector.addItems(list(self.processed_data.keys()))
    ...
    def plot_gravity_level(
        ...
        if original_file_path:
            ...
            graph_path = os.path.join(original_dir, f"{file_name_without_ext}_gravity_level_graph.png")
    ...
    def update_standard_table(self):
        ...
        for row, (file_name, data) in enumerate(self.processed_data.items()):
            ...
            min_mean_drag_shield, min_time_drag_shield, min_std_drag_shield = calculate_statistics(
                data["filtered_gravity_level_drag_shield"], data["filtered_time"], self.config
            )

            # テーブルにデータを設定
            self.table.setItem(row, 0, QTableWidgetItem(file_name))
            self.table.setItem(row, 1, QTableWidgetItem(f"{min_time_inner_capsule:.3f}"))
            self.table.setItem(row, 2, QTableWidgetItem(f"{min_mean_inner_capsule:.4f}"))
            self.table.setItem(row, 3, QTableWidgetItem(f"{min_std_inner_capsule:.4f}"))
            self.table.setItem(row, 4, QTableWidgetItem(f"{min_time_drag_shield:.3f}"))
            self.table.setItem(row, 5, QTableWidgetItem(f"{min_mean_drag_shield:.4f}"))
            self.table.setItem(row, 6, QTableWidgetItem(f"{min_std_drag_shield:.4f}"))
    ...
    # すべてのデータセットに対してG-quality解析を実行
    def perform_g_quality_analysis_for_all_datasets(self):
        """
        すべてのデータセットに対してG-quality解析を実行するメソッド
        """
    ...
    def plot_g_quality_data(self, g_quality_data, file_name):
        ...
        if file_name in self.file_paths:
            output_dir = os.path.dirname(self.file_paths[file_name])
            graph_path = os.path.join(output_dir, f"{file_name}_g_quality_analysis.png")
            self.figure.savefig(graph_path, dpi=300, bbox_inches="tight")
