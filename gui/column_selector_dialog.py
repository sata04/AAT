...
class ColumnSelectorDialog(QDialog):
    ...
    def __init__(self, time_columns, accel_columns, parent=None):
        ...
        # チェックボックスとコンボの連動（視覚フィードバック付き）
        def _update_combo_style(combo, enabled):
            combo.setEnabled(enabled)
            if not enabled:
                combo.setStyleSheet(f"color: {Colors.TEXT_DISABLED};")
            else:
                combo.setStyleSheet("")

        self.use_inner_toggle.toggled.connect(lambda checked: _update_combo_style(self.inner_combo, checked))
        self.use_drag_toggle.toggled.connect(lambda checked: _update_combo_style(self.drag_combo, checked))

        # 初期状態の視覚フィードバックを適用
        if not self.use_inner_toggle.isChecked():
            self.inner_combo.setStyleSheet(f"color: {Colors.TEXT_DISABLED};")
        if not self.use_drag_toggle.isChecked():
            self.drag_combo.setStyleSheet(f"color: {Colors.TEXT_DISABLED};")
